...
def detectFaces(image_name):
    ...
    face_cascade = cv2.CascadeClassifier("I:\7_Software\opencv\data\haarcascades\haarcascade_frontalface_default.xml")
    if img.ndim == 3:
...
def detectEyes(image_name):
    eye_cascade = cv2.CascadeClassifier(r'I:\7_Software\opencv\data\haarcascades\haarcascade_eye.xml')
    faces = detectFaces(image_name)
...
def detectSmiles(image_name):
    img = cv2.imread(image_name)
    smiles_cascade = cv2.CascadeClassifier(r"I:\7_Software\opencv\data\haarcascades\haarcascade_smile.xml")
    if img.ndim == 3:
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    else:
        gray = img #if语句：如果img维度为3，说明不是灰度图，先转化为灰度图gray，如果不为3，也就是2，原图就是灰度图
